...
class View {
    ...
    public static function initView () {
        ...
        $menu_config = Config::getSection('menu');
        foreach ($menu_config as $value) {
            $menu_array = explode ('::',$value);
    ...
    public static function showPage ($page=null,$action=null,$id=null) {
        if(empty($page)) $page=Router::getRouteParam('page');
        if(empty($action)) $action=Router::getRouteParam('action');
        if(empty($id)) $id=Router::getRouteParam('id');
