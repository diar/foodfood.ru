...
/**
 * Класс обработки URL'ов вида /restaurant/
 *
 */
class ActionBlog extends Action {
	 ...
	 * @var unknown_type
	 */
	protected $sMenuItemSelect='restaurant';
	/**
	...
	public function Init() {
		...
		$this->sMenuSubBlogUrl=Router::GetPath('restaurant');
		/**
	...
	/**
	 * Регистрируем евенты, по сути определяем УРЛы вида /restaurant/.../
	 *
	 */
	...
	protected function EventTopics() {
		 ...
		 */
		$aPaging=$this->Viewer_MakePaging($aResult['count'],$iPage,Config::Get('module.topic.per_page'),4,Router::GetPath('restaurant').$sShowType);
		/**
		 * Вызов хуков
		 */
	...
	protected function EventShowTopic() {
		...
		if ($this->GetParamEventMatch(0,1)) {
			...
			$this->sMenuItemSelect='restaurant';
		} else {
	...
	protected function SendBlogInvite($oBlog,$oUser) {
		...
		$aPath=array(
			'accept'=>Router::GetPath('restaurant').'invite/accept/?code='.$sCode,
			'reject'=>Router::GetPath('restaurant').'invite/reject/?code='.$sCode
		);
