	...
	<div class="lite-center register">
		<form action="{router page='registration'}" method="POST">
			...
			<span class="input-note">{$aLang.registration_login_notice}</span></p>
                        ...
                        <label for="phone">{$aLang.registration_phone}:</label><br />
			<p><input type="text" class="input-text" id="phone" value="{$_aRequest.phone}" name="phone"/><br />
			<span class="input-note">{$aLang.registration_phone_notice}</span></p>
