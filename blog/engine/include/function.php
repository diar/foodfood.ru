...
function func_check($sValue,$sParam,$iMin=1,$iMax=100) {
    ...
    switch($sParam) {
        ...
        case 'float': if (preg_match("/^[\-]?\d+[\.]?\d*$/",$sValue)) {
            ...
            ;
        case 'mail': if (preg_match("/^[\da-z\_\-\.\+]+@[\da-z_\-\.]+\.[a-z]{2,5}$/i",$sValue)) {
                return true;
            } break

            ;
        case 'login': if (preg_match("/^[\da-zа-я\_\-\.]{".$iMin.','.$iMax."}$/iu",$sValue)) {
                return true;
            } break
        ...
        case 'text': if (mb_strlen($sValue,'UTF-8')>=$iMin and mb_strlen($sValue,'UTF-8')<=$iMax) {
            ...
            ;
        case 'phone':
            $sValue = preg_replace("/^[7-8]/",'+7',$sValue);
            if (preg_match('/^[+]?[[:digit:]]{5,12}$/i',$sValue))
                return true;
            break;
        default:
            return false;
