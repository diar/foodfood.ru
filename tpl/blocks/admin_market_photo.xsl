...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/">
        <script type="text/javascript" src="js/market_photos.js"></script>
        <script type="text/javascript">
            <xsl:text>dish_id = </xsl:text><xsl:value-of select="//dish_id" />;
        </script>
        <div id="photos_container">
            <div>Нажмите для загрузки фотографии</div>
            <form id="imgform" method="post" enctype="multipart/form-data"
                  action="/admin/admin.php?page=marketPhotos&amp;action=add&amp;id={//dish_id}" target="frame">
