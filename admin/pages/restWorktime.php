...
class restWorktime extends AdminModule {
    ...
    public static function save() {
        ...
        foreach ($rows as $post) {
            if (strlen($post['start_hour'])<2) $post['start_hour']='0'.$post['start_hour'];
            if (strlen($post['end_hour'])<2) $post['end_hour']='0'.$post['end_hour'];
            if (strlen($post['start_minute'])<2) $post['start_minute']='0'.$post['start_minute'];
            if (strlen($post['end_minute'])<2) $post['end_minute']='0'.$post['end_minute'];
